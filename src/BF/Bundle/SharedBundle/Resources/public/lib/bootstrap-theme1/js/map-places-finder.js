...
$(document).ready(function(){	
	...
	$( "#slider-range2" ).slider({
		...
		slide: function( event, ui ) {
			$( "#prange span" ).html( "$" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] );
		}
	});
	$( "#prange span" ).html( "$" + $( "#slider-range2" ).slider( "values", 0 ) +
	...
	$( "#slider-range3" ).slider({
		...
		values: [ 1, 4 ],
		slide: function( event, ui ) {
			$( "#srange span" ).html( ui.values[ 0 ] + " - " + ui.values[ 1 ] );
		}
	});
	$( "#srange span" ).html( $( "#slider-range3" ).slider( "values", 0 ) +
		" - " + $( "#slider-range3" ).slider( "values", 1 ) );

	$('#mapTitle span').html('Indonesia');
	$('#countryData').change(function() {
	   $('#mapTitle span').html($('.result-selected').text());
	});
});
...
map.setZoom(15);
search();
$('#mylistingContainer, #myFilterResult, #myFriendSuggest').show();
}
